...
export async function generateMetadata({ params }) {
  ...
  if (!selectedCategory) {
    return {
      ...
      description: "Опис категорії не знайдено",
    };
  }

  // Перевірка на наявність seoTitle та seoDescription для поточної мови
  const title = selectedCategory?.seotitle?.[lng] || "Категорія";
  const description =
    selectedCategory?.ceodescriptions?.[lng] || "Опис цієї категорії.";
